...
ENVIRONMENT = [(i, i) for i in (u"dev", u"test", u"pre", u"prod")]
...
class gitCode(models.Model):
    uuid = UUIDField(auto=True, primary_key=True)
    codePath = models.CharField(verbose_name=u'代码存放目录', max_length=64, help_text=u'存放路径如/data/code')
    # codeserver = models.CharField(verbose_name=u'发布代码服务器', max_length=64, help_text=u'线上发布代码仓库或aws发布仓库')
    codeserver = models.CharField(verbose_name=u'服务代码git地址URL', max_length=64, help_text=u'github or gitlab url')
    codename = models.CharField(verbose_name=u'本地服务代码目录名', max_length=64, help_text=u'本地服务代码目录名')
    # codeFqdn = models.CharField(verbose_name=u'salt主机名', max_length=64)
    codeFqdn = models.CharField(verbose_name=u'源码存放主机 front in run host , java end at master host', max_length=64, help_text=u'front and end code')
    environ = models.CharField(verbose_name=u'运行环境 prod stage test dev', max_length=8, help_text=u'runtime belog to environment')
...
class project_swan(models.Model):
    ...
    """
    uuid = UUIDField(auto=True, primary_key=True)
    swan_name = models.CharField(max_length=100, verbose_name=u"发布服务名称", help_text=u"发布时同一项目有多个按布按钮，此处设置为发布选项名称")
