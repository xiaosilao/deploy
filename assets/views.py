...
def host_add(request):
    ...
    if request.method == 'POST':
        ...
        if uf_post.is_valid():
            ...
            print "zw %s -- physics %s" % (zw, physics)
            ...
            if zabbix_on and status == 1:
                print "zabbix_on true"
                zabbix_host_add(request)
            smg = u'主机 %s 添加成功!' % ip
            print "type : %s" % type(zw)
            print "zw-mac %s --- %s ---- %d -- %s" % (zw.mac,ip,status,smg)
...
def ip_list(request):
    """ ip列表 """
    idcs = IDC.objects.all()
    rzjf_delixi = get_object_or_404(IDC, name='rzjf_delixi')
    active_111 = IpList.objects.filter(idc=rzjf_delixi, network='10.1.1.0/24', status=1)
    unactive_111 = IpList.objects.filter(idc=rzjf_delixi, network='10.1.1.0/24', status=1)
    active_112 = IpList.objects.filter(idc=rzjf_delixi, network='192.168.10.0/24', status=1)
    unactive_112 = IpList.objects.filter(idc=rzjf_delixi, network='192.168.10.0/24', status=1)
    # active_113 = IpList.objects.filter(idc=rzjf_delixi, network='192.168.113.0/24', status=1)
    # unactive_113 = IpList.objects.filter(idc=rzjf_delixi, network='192.168.113.0/24', status=1)
