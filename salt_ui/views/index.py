...
def salt_index_new(request):
    ...
    start_time, stop_time, week_start_time, month_start_time = Date_time()
    # print month_start_time, type(month_start_time)
    ...
    week_swan_push_data = SwanLog.objects.filter(swan_datetime__gte=week_start_time)
    week_swan_data = {}
    for j in week_swan_push_data:
        project_name = j.project_name
        if week_swan_data.get(project_name, False):
            week_swan_data[j.project_name]["Count"] += 1
        else:
            week_swan_data[j.project_name] = {"Count": 1, "uuid": j.project_uuid}
    month_swan_push_data = SwanLog.objects.filter(swan_datetime__gte=month_start_time)
    month_swan_data = {}
    for k in month_swan_push_data:
        project_name = k.project_name
        if month_swan_data.get(project_name, False):
            month_swan_data[k.project_name]["Count"] += 1
        else:
            month_swan_data[k.project_name] = {"Count": 1, "uuid": k.project_uuid}
